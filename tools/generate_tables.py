...
if __name__ == "__main__":
    ...
    # python
    path    = join(proj, "python", "tables.py")
    bracket = ("[", "]")
    writer  = Writer(path)
    writer.write("", "S"   , bracket, t.sbox    , "",     )
    writer.write("", "Si"  , bracket, t.sbox_inv, "",     )
    writer.write("", "Rcon", bracket, t.rcon    , "", 6, 8)
    writer.write("", "T0"  , bracket, t.t0      , "", 8, 8)
    writer.write("", "T1"  , bracket, t.t1      , "", 8, 8)
    writer.write("", "T2"  , bracket, t.t2      , "", 8, 8)
    writer.write("", "T3"  , bracket, t.t3      , "", 8, 8)
    writer.write("", "T4"  , bracket, t.t4      , "", 8, 8)
    writer.write("", "T5"  , bracket, t.t5      , "", 8, 8)
    writer.write("", "T6"  , bracket, t.t6      , "", 8, 8)
    writer.write("", "T7"  , bracket, t.t7      , "", 8, 8)
    writer.write("", "U0"  , bracket, t.u0      , "", 8, 8)
    writer.write("", "U1"  , bracket, t.u1      , "", 8, 8)
    writer.write("", "U2"  , bracket, t.u2      , "", 8, 8)
    writer.write("", "U3"  , bracket, t.u3      , "", 8, 8)
    writer.close()

    # c
    path    = join(proj, "c", "tables.h")
    bracket = ("{", "}")
    writer  = Writer(path)
    writer.write_direct(
        "#ifndef TABLES_H\n"
    ...
    writer.write("const uint8_t " , "S[256]"  , bracket, t.sbox    , ";",     )
    writer.write("const uint8_t " , "Si[256]" , bracket, t.sbox_inv, ";",     )
    writer.write("const uint32_t ", "Rcon[30]", bracket, t.rcon    , ";", 6, 8)
    writer.write("const uint32_t ", "T0[256]" , bracket, t.t0      , ";", 8, 8)
    writer.write("const uint32_t ", "T1[256]" , bracket, t.t1      , ";", 8, 8)
    writer.write("const uint32_t ", "T2[256]" , bracket, t.t2      , ";", 8, 8)
    writer.write("const uint32_t ", "T3[256]" , bracket, t.t3      , ";", 8, 8)
    writer.write("const uint32_t ", "T4[256]" , bracket, t.t4      , ";", 8, 8)
    writer.write("const uint32_t ", "T5[256]" , bracket, t.t5      , ";", 8, 8)
    writer.write("const uint32_t ", "T6[256]" , bracket, t.t6      , ";", 8, 8)
    writer.write("const uint32_t ", "T7[256]" , bracket, t.t7      , ";", 8, 8)
    writer.write("const uint32_t ", "U0[256]" , bracket, t.u0      , ";", 8, 8)
    writer.write("const uint32_t ", "U1[256]" , bracket, t.u1      , ";", 8, 8)
    writer.write("const uint32_t ", "U2[256]" , bracket, t.u2      , ";", 8, 8)
    writer.write("const uint32_t ", "U3[256]" , bracket, t.u3      , ";", 8, 8)
    writer.write_direct(
        "#endif // TABLES_H\n"
        "\n"
    )
